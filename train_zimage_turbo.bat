@echo off
REM Z-Image Turbo training optimized for NVIDIA RTX 5090
REM Fast VAE for real-time applications
...
echo 1. Ultra-Fast (768x768, batch=32) - ~18GB VRAM (5x speed)
echo 2. Fast Standard (1024x1024, batch=24) - ~24GB VRAM (3x speed) [Recommended]
echo 3. High Resolution (1536x1536, batch=12) - ~27GB VRAM (2x speed)
echo 4. Ultra Resolution (2048x2048, batch=6) - ~29GB VRAM (1.8x speed)
...
if "%profile_option%"=="1" (
    ...
    set /p BATCH_SIZE="Batch size (e.g., 24): "
    set /p Z_CHANNELS="Z channels (e.g., 8): "
    set /p LEARNING_RATE="Learning rate (e.g., 2e-4): "
) else (
...
REM Start training with Z-Image Turbo optimizations
python train_vae.py ^
